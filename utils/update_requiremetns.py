...
def create_new_issue(issue_title, issue_body, req_info_list, req_attributes, labels):
    ...

def update_feature_requirements_issue(issue_title, issue_body, req_info_list, req_attributes, labels):
    owner = "orm-backend-final-project-2-2"
    repo = "final-project-server"
    token = pat

    url = f"https://api.github.com/repos/{owner}/{repo}/issues/lab
    response = requests.get(url, headers={"Authorization": f"token {token}"})

    if response.status_code == 200:
        pdate_existing_issue(issue_title, issue_body, req_info_list, req_attributes, labels)
    else:
...
def update_app_requirements(app_name, req_file):
    ...
    with open(req_file, 'r') as f:
        ...
        for feature in app_requirements_info:
            ...
            issue_body = create_markdown_table(app_requirements_info[feature], req_attributes)

            update_feature_requirements_issue(issue_title, issue_body, app_requirements_info[feature], req_attributes, labels)

def update_requirements(raw_changed_files):
    changed_req_files = [file for file in raw_changed_files.split('\n') if file.endswith('_requirements.md')]
